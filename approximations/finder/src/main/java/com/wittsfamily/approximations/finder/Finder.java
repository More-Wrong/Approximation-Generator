...
public class Finder {
	public static void main(String[] args) throws IOException, ParseException {
		...
		String number;
		if (numPos == -1) {
			number = (String) JOptionPane.showInputDialog(null, "Please enter a number to approximate", "Enter number",
					JOptionPane.PLAIN_MESSAGE);
		} else {
			number = args[numPos];
		}
		number = number.replaceAll("[, ]", "").replaceAll("[-\u2212\u002D\uFE63\uFF0D]", "-")
				.replaceAll("[*x\u2062]", "*").replaceFirst("\\*10\\^", "*10").replaceFirst("\\*10", "e");
		...
		int depth;
		if (depthPos == -1) {
			depth = Integer.parseInt((String) JOptionPane.showInputDialog(null, "Please enter a depth of optimisation",
					"Enter Optimisation", JOptionPane.PLAIN_MESSAGE));
		} else {
		...
		for (Expression value : vals) {
			...
			if (precision == ApfloatWithInf.INF) {
				String text = target.toString(true) + "\\approx " + ltxStr
						+ " \\\\\\text{To system precision (1/10^{50}, \\pm rounding error)}";
				laTeXStrs.add(text);
				...
				text = text + precision.divide(new Apfloat(Math.pow(10, log10.intValue()), 3)).toString()
						+ "\\times 10^{" + log10.intValue() + "}";
				laTeXStrs.add(text);
			}
		}
