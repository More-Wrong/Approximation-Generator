		...
		for (Iterator<CleaningExpression> iterator = expressions.iterator(); iterator.hasNext();) {
			...
			if (child.getType() == CleaningExpressionType.INTEGER) {
				...
				if (child.getExpressions().get(0).getType() == CleaningExpressionType.INTEGER
						&& child.getExpressions().get(1).getType() == CleaningExpressionType.REAL) {
					int num = ((CleaningInteger) child.getExpressions().get(0)).asInt();
					for (int i = 0; i < num; i++) {
						reals.add((CleaningReal) child.getExpressions().get(1));
					}
					iterator.remove();
				} else if (child.getExpressions().get(1).getType() == CleaningExpressionType.INTEGER
				...
				if (cc.getType() == CleaningExpressionType.INTEGER) {
					...
					if (cc.getExpressions().get(0).getType() == CleaningExpressionType.INTEGER
							&& cc.getExpressions().get(1).getType() == CleaningExpressionType.REAL) {
						int num = ((CleaningInteger) cc.getExpressions().get(0)).asInt();
						for (int i = 0; i < num; i++) {
							negReals.add((CleaningReal) cc.getExpressions().get(1));
						}
						iterator.remove();
					} else if (cc.getExpressions().get(1).getType() == CleaningExpressionType.INTEGER
							&& cc.getExpressions().get(0).getType() == CleaningExpressionType.REAL) {
